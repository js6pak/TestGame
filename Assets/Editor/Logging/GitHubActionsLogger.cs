        ...
        public override void GenerateBuildSummary(BuildReport buildReport, Stopwatch stopwatch)
        {
            ...
            using (var summaryWriter = new StreamWriter(summary, true, Encoding.UTF8))
            {
                try
                {
                    ...
                    {
                        summaryWriter.WriteLine("Errors: {0}", buildReport.summary.totalErrors);
                    }

                    if (buildReport.summary.totalWarnings > 0)
                    {
                        summaryWriter.WriteLine("Warnings: {0}", buildReport.summary.totalWarnings);
                    }

                    if (stopwatch != null)
                    {
                        summaryWriter.WriteLine("Total duration: {0:g}", stopwatch.Elapsed);
                    }

                    summaryWriter.WriteLine("Size: {0}", FormatFileSize(buildReport.summary.totalSize));
                    summaryWriter.WriteLine("Output Path: {0}", buildReport.summary.outputPath);
                    summaryWriter.WriteLine("");
                    ...
                    Func<BuildStepMessage, string> ProcessLogMessage = message =>
                    {
                        ...
                        switch (message.type)
                        {
                            ...
                            case LogType.Exception:
                                return string.Format("| :boom: {0} | {1} |", message.type, logMessage);
                            case LogType.Warning:
                                return string.Format("| :warning: {0} | {1} |", message.type, logMessage);
                            case LogType.Log:
                            default:
                                return string.Format("| {0} | {1} |", message.type, logMessage);
                        }
                    };

                    Action<List<BuildStepMessage>> WriteLogSummary = logs =>
                    {
                        if (logs.Count == 0)
                        {
                            return;
                        ...
                        if (isErrorMessages)
                        {
                            summaryWriter.WriteLine("<details open><summary>Errors ({0})</summary>", logs.Count);
                        }
                        else if (isWarningMessages)
                        {
                            summaryWriter.WriteLine("<details><summary>Warnings ({0})</summary>", logs.Count);
                        }
                        else
                        {
                            summaryWriter.WriteLine("<details><summary>Logs ({0})</summary>", logs.Count);
                        }
                    ...
                    switch (buildReport.summary.result)
                    {
                        case BuildResult.Succeeded:
                            Debug.Log(string.Format("{0}", buildResultMessage));
                            break;
                        case BuildResult.Unknown:
                        case BuildResult.Cancelled:
                            Debug.Log(string.Format("{0}{1}{2}", WarningColor, buildResultMessage, ResetColor));
                            break;
                        case BuildResult.Failed:
                            Debug.Log(string.Format("{0}{1}{2}", ErrorColor, buildResultMessage, ResetColor));
                            break;
                        default:
