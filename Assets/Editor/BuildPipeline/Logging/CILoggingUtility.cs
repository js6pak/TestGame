...
namespace Buildalon.Editor.BuildPipeline.Logging
{
    ...
    public static class CILoggingUtility
    {
        ...
        private static bool loggingEnabled = ApplicationEx.isBatchMode;
        ...
        {
            if (!StringEx.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("TF_BUILD", EnvironmentVariableTarget.Process)))
            {
                Logger = new AzurePipelinesLogger();
            }

            if (!StringEx.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("GITHUB_ACTIONS", EnvironmentVariableTarget.Process)))
            {
                Logger = new GitHubActionsLogger();
            }
        }

        public static void GenerateBuildReport(BuildReport buildReport, Stopwatch stopwatch)
        {
            if (Logger != null) Logger.GenerateBuildSummary(buildReport, stopwatch);
        }
    }
}
