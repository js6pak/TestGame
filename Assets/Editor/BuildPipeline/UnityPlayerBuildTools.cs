...
#if UNITY_2018_1_OR_NEWER
        Action<BuildReport> CommandLineBuildReportCallback = (BuildReport postProcessBuildReport) =>
        {
...
// #endif // UNITY_6000_0_OR_NEWER
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        // set build number
        if (!StringEx.IsNullOrWhiteSpace(buildInfo.BuildNumber))
        {
#if PLATFORM_ANDROID
                int code;
                if (int.TryParse(buildInfo.BuildNumber, out code))
                {
    ...
    {
        if (buildInfo != null) buildInfo.OnPostProcessBuild(report);
        if (OnBuildCompletedWithSummary != null) OnBuildCompletedWithSummary.Invoke(report);
    }
